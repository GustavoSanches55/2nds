...
def create_random_words(palavras = ['prova', 'seminario', 'dificuldade', 'facilidade', "none", "none", "none"]):
...
def create_df(palavras_mais_usadas):
    ...
    df_final['color'] = df_final['carater'].map({"disposto-bom": '#F7E967', 'disposto-ruim': '#F1433F', 'indisposto-bom': '#A9CF54', 'indisposto-ruim': '#70B7BA'})


    df_final['palavras_mais_usadas'] = palavras_mais_usadas
    df_final['color'] = df_final['carater'].map({"disposto-bom": '#F7E967', 'disposto-ruim': '#F1433F', 'indisposto-bom': '#A9CF54', 'indisposto-ruim': '#70B7BA'})

    return df_final, df_disciplina_assunto, professor

def create_teacher_graph(df_final, df_disciplina_assunto, teacher_id, palavras_mais_usadas):
    grafo = Graph()
    ...
    grafo.add_group_nodes({"disposto-bom": '#F7E967', 'disposto-ruim': '#F1433F', 'indisposto-bom': '#A9CF54', 'indisposto-ruim': '#70B7BA'}, df_disciplina_assunto['nome_assunto'])
    ...
    grafo.save_graph("grafo_professor_" + str(teacher_id) + ".html")

def create_general_graph(df_final, df_disciplina_assunto,professor):
    grafo = Graph()
    ...
    grafo.save_graph("general_graph.html")
